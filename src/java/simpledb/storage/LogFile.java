...
public class LogFile {
    ...
    static final int ABORT_RECORD = 1; // 事务发生abort时调用，由Transaction.transactionComplete()调用LogFile中的logAbort()进行写入。
    static final int COMMIT_RECORD = 2; // 事务commit时调用，由Transaction.transactionComplete()调用LogFile中的logCommit()进行写入。同时调用logCommit()会调用force()会强制将还在channel缓冲的数据刷到disk。
    ...
    long currentOffset = -1;//当前偏移量，用来读取日志时用
    //    int pageSize;
    int totalRecords = 0; // 记录的日志数

    // 记录这当前活跃的事务第一个日志的偏移量，因为不活跃的已经提交刷盘了不用记录
